...
1. Extract the original with the plugin's `scripts/clip.sh <input> [category]` and save it under `_sources/{category}/`. The input can be a URL or a local file path. clip.sh dispatches three ways: PDFs (local path with `%PDF` magic bytes or `https://...pdf` URL) take the pdftotext path with `OMO_PDF_LAYOUT` env override; YouTube URLs (`youtube.com`/`youtu.be`/`m.youtube.com`) take the yt-dlp subtitle path with `OMO_YT_SUB_LANG` env override (default `ko,en`); everything else takes the Playwright + Defuddle web path (no AI processing). If Defuddle cannot identify the article body on the web path, clip.sh exits 2 with the rendered Playwright HTML preserved and the calling skill performs LLM extraction from that HTML — never from a fresh fetch, so the rendered DOM is never thrown away. The PDF and YouTube paths use only exit 0/1 (no LLM fallback applies — those formats are not HTML to begin with).
